...
senseData = mngr.execute_mysql("SELECT datetime, val FROM plant_events WHERE code=8 AND plant=2 LIMIT 10", [])
sensex = [entry[0] for entry in senseData]
sensey = [int(entry[1]) / 100.0 for entry in senseData]
print(sensex[0], "Length:", len(sensex[0]))


manualData = mngr.execute_mysql("SELECT datetime, val FROM plant_events WHERE code=2 AND plant=2 LIMIT 10", [])
manualx = [entry[0] for entry in manualData]
manualy = [int(entry[1]) / 100.0 for entry in manualData]
print(manualx[0], "Length:", len(manualx[0]))
